...
async def resolve_continue_url(
    ...
    """
    Функція, яка визначае URL для редіректу після аутентифікації на основі EDM-прапора PossibilityForPreview.
    Якщо встановлений пратор вимогає перегляд та підтвердення доказу, то повертає URL з PreviewLocation,
    ...
    if edm.get("doc", {}).get("PossibilityForPreview"):
        pl = edm.get("doc", {}).get("PreviewLocation")
        _logger.debug(f"Перенаправляємо на предперегляд: {PREVIEW_URL}?{returnurl}")
